...
namespace eval steiner {
   namespace eval fortune {
	...
	proc public {nick uhost hand chan arg} {
		...
		if {[channel get $chan steiner] && [channel get $chan fortune]} { puthelp "PRIVMSG $targ :[fortune $targ $fnick]" } else { puthelp "PRIVMSG $nick :[fortune $targ $fnick]" }
	}
	...
	proc fortune { targ nick } {
		if { [catch {
			...
			catch { exec $steiner::settings::fortune::path {*}$steiner::settings::fortune::options | sed "s/--.*//" | tr {\n\t} { } } output
